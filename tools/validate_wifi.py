...
def load(path):
    try: return json.loads(path.read_text(encoding="utf-8"))
        ...
        errors.append(f"{path}: invalid JSON: {ex}"); return None
...
def wildcard(value, pattern):
    return re.fullmatch(re.escape(pattern).replace(r"\*", ".*"), value, re.I) is not None
...
def validate_capabilities(profile, path):
    caps = profile.get("climateCapabilities") if profile.get("deviceType") == "AirConditioner" else profile.get("tvCapabilities")
    if not isinstance(caps, dict): errors.append(f"{path}: capabilities object required"); return
    if profile.get("deviceType") == "AirConditioner":
        temp = caps.get("targetTemperature")
        if temp:
            mn,mx,step=temp.get("min"),temp.get("max"),temp.get("step")
            if not all(isinstance(x,(int,float)) for x in (mn,mx,step)) or mn >= mx or step <= 0: errors.append(f"{path}: invalid targetTemperature capability")
        if caps.get("modes") is not None and not isinstance(caps.get("modes"), list): errors.append(f"{path}: modes must be array")
...
for entry in index.get("profiles",[]):
    ...
    if profile.get("deviceType") not in {"AirConditioner","Television"}: errors.append(f"{p}: unsupported deviceType")
...
if not suite: errors.append("wifi/validation-suite.json required")
    ...
    for test in suite.get("commandTests",[]):
        did=test.get("driverId"); driver=drivers.get(did)
    ...
    for test in suite.get("negativeTests",[]):
        ...
        for state_name in test.get("missingExpected",[]):
            expr=op.get("stateMap",{}).get(state_name)
            try:
                extract_json_path(test.get("response"),expr)
                errors.append(f"negativeTest {test.get('name')}: {state_name} unexpectedly resolved")
            except Exception: pass
    def score_profile(p, brand, model):
        if p.get("deviceType") == "": return -1
